...
public readonly struct NoiseEvent
{
    ...
    /// <remarks>
    /// 여기서는 거리만 봅니다. 벽이나 엄폐물에 의한 차폐는 이 판정 뒤에 듣는 쪽이
    /// <see cref="NoiseManager"/>에게 물어 감지 강도를 깎는 방식으로 적용하며, 완전히 막힌 경우에만
    /// 가청을 취소합니다. 그래서 "도달 거리가 곧 들리는 거리"라는 이 구조체의 규약은 그대로 유지됩니다.
    ///
    /// <b>기획 편차</b>: 공용 `적 시스템` v0.2 §5.4.4는 "거리 감쇠 외에 벽이나 엄폐물에 의한 장애물 감쇠는
    /// 적용하지 않는다"고 정합니다. 사용자 지시로 코드를 먼저 바꾼 상태이며, 문서 개정 전까지 이 편차를
    /// 문서 쪽 실수로 오해해 되돌리지 말 것.
    ///
